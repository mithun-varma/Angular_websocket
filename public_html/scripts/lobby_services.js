 ...
 */
var services = angular.module('LobbyServices', ['server_protocols']);

var lobbyService = function (serverProtocolBuilder) {
    ...
    this. processPlayPool = function(data) {
        var gameDefinitions;
    ...
    this.processPlayStake = function(data) {
        var gameDefinitions;
        var gameDefinitionDetails;
        var gameDefinition;
        var definitionDetails;
    ...
    this.processRealStake = function(data) {
        var gameDefinitions;
        var gameDefinitionDetails;
        var gameDefinition;
        var definitionDetails;
    ...
    this.processRealPool = function(data) {
        var gameDefinitions;
        var gameDefinitionDetails;
        var gameDefinition;
        var definitionDetails;
    ...
    this.processTournamentData = function(data) {
    ...
    this.addRealTournament = function(tourneyDetails, isInitailTournament) {
    ...
    this.addPlayTournament = function(tourneyDetails, isInitailTournament) {
        var context = this.getLobbyContext();
        var tournament = new Tournament();
    ...
    this.updateRegisteredTourneys = function(data)
    {
        /**
    ...
    this.updateUnRegisteredTourney = function(data) {
    ...
    this.removeGameDefinition = function(definitionID, typeOfGame) {
        ...
        } else if (typeOfGame === this.PLAYSTAKE) {
            console.error(" We are not Handling the Play stake data ");
        } else if (typeOfGame === this.REALSTAKE) {
            if (!isRemoved) {
    ...
    this.addGameDefinition = function(gamePlayType, gameDefinitionDetails) {
        var index = 0;
        var definitionId;
        var gameType;
        var bet;
    ...
    this.showIpblockMessage = function(message) {
        /*
         *  Here we need to show the alert message to the player
         */

    };
    this.processServerDetails = function(message) {
    ...
    this.setPseudoExpired = function() {
...
// curl - www.ace2three.com * fav:321:N:409, 7, 127, 11 * /
        ...
        if (status === 3) {
            ...
            for (var index in loginDetails) {
                ...
                if (key === "n") {
                    ...
                    if (value === "N") {
                        player.setPanUpdated(false);
                    } else if (value === "Y") {
                        player.setPanUpdated(true);
                    }
                } else if (key === "PSD") {
                    value = value.trim();
                    if (value === "true") {
            ...
            serverProtocolBuilder.lack();
            //context.getLobbyCommunicator().getCommunicationChannel().sendMessage("lack#");
            console.log("Running login " + player.getUserName());
        } else {
            var message;
            loginDetails = details[1].split(':');
            this.setLoginDate();
            if (loginDetails[0] === 'r') {
                if (loginDetails[1].toString().toLowerCase() === 'noemail') {
                    showEmailVerificationWindow();
                    if (context.player) {
                        context.player.userCode = loginDetails[2];
                    }
                } else {
                    if (loginDetails[1] == '1000') {
                        if (!context.lobbyCanvas.loginWindow) {
                            context.lobbyCanvas.addLoginWindow();
                        }
                        context.lobbyCanvas.loginWindow.invalidMessage('Your account has been disabled. Kindly contact customer support for further information.');
                        context.lobbyCanvas.loginWindow.errorMessage.y = context.lobbyCanvas.loginWindow.errorMessage.y - 12;
                        return;
                    } else {
                        message = loginDetails[1];
                    }
                }
    ...
    this.processLoginSession = function(value) {
        var context = this.getLobbyContext();       
        if (context.getSessionId() !== value) {          
            context.setOldSessionId(context.getSessionId());
            context.setSessionId(value);           
        }
    };

    this.processLackData = function(lackGamesArray) {
        var context = ApplicationContext.getContextInstance();
        var player = context.getPlayer();
        if (!lackGamesArray) {
...
//   }
    ...
    this.setPseudoToPremium = function() {
        var context = ApplicationContext.getContextInstance();
        var player = context.getPlayer();
        player.setIsPseudoPlayer(false);
    };
    ...
    this.updateGamesStatus = function(definitions, gamesStatus) {
        var isClosed;
    ...
    this.updatePlayPoolData = function(data) {
        var context = this.getLobbyContext();
    ...
    this.updateRealPoolData = function(data) {
    ...
    this.updatePlayStakeData = function(data)
    {
        var context = this.getLobbyContext();
        if (data.indexOf("$") === data.length - 1) {
            data = data.substring(0, data.length - 1);
    ...
    this.updateRealStakeData = function(data)
    {
    ...
    this.showLogoutMessage = function(value) {
        var message;
        var applicationContext = ApplicationContext.getContextInstance();
    ...
    this.updatePanStatus = function(value) {
        var context = ApplicationContext.getContextInstance();
        var player = context.getPlayer();
    ...
    this.setPlayerAceLevelDetails = function(aceLevelDetailsVO) {
        /*
         * Here we need to handle the functionality for the player's "AceLevel" related.   
         */
    };

    this.reloadChips = function(value) {
    ...
    this.updateChips = function(data) {
        /*var aceLevel;
         var acePoints;
         var aceStars;*/
        var context = ApplicationContext.getContextInstance();
    ...
    this.setLobbyScope = function(value) {
        this.lobbyScope = value;
    };

    this.getLobbyScope = function() {
        return this.lobbyScope;
    };

    this.setLobbyContext = function(value) {
        this.lobbyModel = value;
    };

    this.getLobbyContext = function() {
...
services.service('lobbyService', ['serverProtocolBuilder',lobbyService]);
